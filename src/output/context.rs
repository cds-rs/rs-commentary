...
impl<'a> RenderContext<'a> {
    pub fn new(
        ...
        // Collect all variables in declaration order, filtering Copy types if configured
        let mut all_vars: Vec<String> = Vec::new();
        for set_ann in set_annotations {
            for entry in &set_ann.set.entries {
                if !all_vars.contains(&entry.name) {
                    // Filter Copy types unless they're borrows (borrows are always interesting)
                    if config.filter_copy_types && entry.is_copy
    ...
    /// Uses SetEntry.is_copy for filtering.
    pub fn filter_entries<'b>(&self, set: &'b OwnershipSet) -> Vec<&'b SetEntry> {
        set.entries
            .iter()
            .filter(|e| {
                if self.config.filter_copy_types && e.is_copy {
                    return false;
                }
                true
            })
    ...
    /// Check if a variable is a Copy type by looking up from SetEntry data.
    /// Searches through all sets to find the most recent entry for this variable.
    pub fn is_copy_type(&self, name: &str) -> bool {
        // Look through sets_by_line to find an entry with this name
        for set in self.sets_by_line.values() {
            for entry in &set.entries {
                if entry.name == name {
                    return entry.is_copy;
                }
            }
        }
        false
    ...
    pub fn get_line_content(&self, line: u32, tracker: &mut AnnotationTracker) -> LineContent {
        ...
        let copy_events: Vec<ProcessedCopy> = raw_copies
            ...
            .map(|c| {
                ...
                ProcessedCopy {
                    from: c.from.clone(),
                    to: c.context.target_name().to_string(),
                    kind: c.kind,
                    still_valid,
                    show_still_valid,
                }
